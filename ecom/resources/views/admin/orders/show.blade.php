...
<div class="row">
            ...
            <div class="card-body">
                @forelse($order->statusHistory as $history)
                    <div class="d-flex justify-content-between mb-2 pb-2 border-bottom">
                        ...
                        <small class="text-muted">{{ $history->created_at->format('M d, Y H:i') }} by {{ $history->changedByUser->name ?? 'System' }}</small>
                    </div>
                @empty
    ...
    <div class="col-md-4">
        ...
        <div class="card mb-4">
            <div class="card-body">
                <h6>Customer</h6>
                <p class="mb-1">{{ $order->customer->name ?? 'N/A' }}</p>
                <p class="text-muted small">{{ $order->customer->email ?? '' }}</p>
            </div>
        </div>
        ...
        @if($order->status->value !== 'cancelled' && $order->status->value !== 'refunded')
            <div class="card">
                <div class="card-body">
                    <h6>Actions</h6>
                    <form method="POST" action="{{ route('admin.orders.cancel', $order->id) }}" onsubmit="return confirm('Cancel this order?')">
                        @csrf
                        <button class="btn btn-outline-danger w-100">Cancel Order</button>
                    </form>
                    <form method="POST" action="{{ route('admin.orders.refund', $order->id) }}" class="mt-2" onsubmit="return confirm('Refund this order?')">
                        @csrf
